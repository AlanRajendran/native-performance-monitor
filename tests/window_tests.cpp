...
int wmain(int argc, wchar_t **argv)
{
    ...
    try
    {
        ...
        require(!FindWindowW(L"NativePerfMonitor.Controller.2.0", nullptr),
        ...
        for (int i = 0; i < 150; ++i)
        {
            control = FindWindowW(L"NativePerfMonitor.Controller.2.0", nullptr);
            if (control)
                break;
            pump(20);
        }
        ...
        EnumWindows(
            [](HWND h, LPARAM l) -> BOOL
            {
                ...
                if (pid == m.pid && (wcscmp(c, L"NativePerfMonitor.Surface.2.0") == 0 ||
                                     wcscmp(c, L"NativePerfMonitor.Strip.2.0") == 0))
        ...
        auto opacity = FindWindowW(L"NativePerfMonitor.Opacity.2.0", nullptr);
        ...
        target = nullptr;
        SendMessageW(control, RegisterWindowMessageW(L"NativePerfMonitor.Stop.6D845648.v2.0"), 0, 0);
    ...
    catch (const std::exception &e)
    {
        ...
        if (process.hProcess)
        {
            auto control = FindWindowW(L"NativePerfMonitor.Controller.2.0", nullptr);
            if (control)
                PostMessageW(control, RegisterWindowMessageW(L"NativePerfMonitor.Stop.6D845648.v2.0"), 0, 0);
