...
int wmain(int argc, wchar_t **argv)
{
    ...
    try
    {
        ...
        require(!FindWindowW(L"NativePerfMonitor.Controller.1.7", nullptr),
        ...
        for (int i = 0; i < 150; ++i)
        {
            control = FindWindowW(L"NativePerfMonitor.Controller.1.7", nullptr);
            if (control)
                break;
            pump(20);
        }
        ...
        EnumWindows(
            [](HWND h, LPARAM l) -> BOOL
            {
                ...
                if (pid == m.pid && (wcscmp(c, L"NativePerfMonitor.Surface.1.7") == 0 ||
                                     wcscmp(c, L"NativePerfMonitor.Strip.1.7") == 0))
        ...
        auto opacity = FindWindowW(L"NativePerfMonitor.Opacity.1.7", nullptr);
        ...
        target = nullptr;
        SendMessageW(control, RegisterWindowMessageW(L"NativePerfMonitor.Stop.6D845648.v1.7"), 0, 0);
    ...
    catch (const std::exception &e)
    {
        ...
        if (process.hProcess)
        {
            auto control = FindWindowW(L"NativePerfMonitor.Controller.1.7", nullptr);
            if (control)
                PostMessageW(control, RegisterWindowMessageW(L"NativePerfMonitor.Stop.6D845648.v1.7"), 0, 0);
