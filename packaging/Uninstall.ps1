...
$ownerId = 'NativePerfMonitor-6D845648-584B-48CE-9904-03E95B0B69E2-v1.6'
$packageDirectory = [IO.Path]::GetFullPath($PackageDirectory)
$defaultData = Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'NativePerfMonitor-1.6'
...
function Finish-Removal([bool]$Success, [string]$Message) {
    ...
    if (-not $Quiet) {
        ...
        [Windows.Forms.MessageBox]::Show($Message, 'Uninstall Native Performance Monitor 1.6', [Windows.Forms.MessageBoxButtons]::OK, $icon) | Out-Null
    }
}
try {
    ...
    $registration = 'Software\Microsoft\Windows\CurrentVersion\Uninstall\NativePerfMonitor-1.6'
    ...
    if ($integration) {
        ...
        foreach ($folder in $shortcutFolders) {
            $link = Assert-OwnedFile $folder 'Native Performance Monitor 1.6.lnk'
    ...
    if (-not $Isolated) {
        ...
        if ($key) {
            try {
                ...
                $actual = [string]$key.GetValue('NativePerfMonitor-1.6', '')
                if ($actual.Equals($expected, [StringComparison]::OrdinalIgnoreCase)) {
                    $key.DeleteValue('NativePerfMonitor-1.6', $false)
                }
            }
            finally { $key.Dispose() }
        }
    ...
    Finish-Removal $true 'Native Performance Monitor 1.6 removed. Owned shortcuts and Installed Apps entry were removed. Unrelated files and earlier versions were kept.'
    exit 0
}
catch {
