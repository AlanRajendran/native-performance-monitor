...
std::filesystem::path defaultDataDirectory()
{
    ...
    return out / L"NativePerfMonitor-2.0";
}
...
static constexpr char currentOwner[] = "NativePerfMonitor-6D845648-584B-48CE-9904-03E95B0B69E2-v2.0";
...
static Settings readSettings(const std::filesystem::path &dir, const char *owner)
{
    Settings r;
    if (hasReparseAncestor(dir) || !ownedData(dir, owner))
        return r;
    std::error_code ec;
    ...
    while (std::getline(f, line))
    {
        ...
        try
        {
            ...
            if (used != v.size())
                continue;
            if (k == "opacity")
                r.opacity = int(std::clamp(n, 10LL, 100LL));
            else if (k == "panel")
                r.panel = n != 0;
            else if (k == "strip")
                r.strip = n != 0;
                ...
                r.panelW = int(std::clamp(n, 300LL, 2400LL));
            else if (k == "panelH")
                r.panelH = int(std::clamp(n, 260LL, 2400LL));
            else if (k == "stripX")
...
{
    return readSettings(dir, currentOwner);
}
bool importPreviousSettings(const std::filesystem::path &dir, Settings &out)
{
    // Each release line keeps its own settings directory beside the others,
    // named after the line and marked with that line's owner id. Newest first.
    static constexpr struct
    {
        const wchar_t *folder;
        const char *owner;
    } previous[] = {
        {L"NativePerfMonitor-1.7", "NativePerfMonitor-6D845648-584B-48CE-9904-03E95B0B69E2-v1.7"},
        {L"NativePerfMonitor-1.6", "NativePerfMonitor-6D845648-584B-48CE-9904-03E95B0B69E2-v1.6"},
    ...
    for (auto &p : previous)
    {
        auto from = dir.parent_path() / p.folder;
        if (from == dir || !std::filesystem::exists(from / L"settings.ini", ec) || !ownedData(from, p.owner))
            continue;
        // Read-only: the previous line's own files are never touched, so it
        // keeps working unchanged if both are installed.
        out = readSettings(from, p.owner);
        // The panel layout changes between lines; its size starts fresh.
        out.panelW = Settings{}.panelW;
        out.panelH = Settings{}.panelH;
        return true;
    }
    return false;
}
...
static std::wstring readStartup()
{
    ...
    if (RegGetValueW(HKEY_CURRENT_USER, runKey, L"NativePerfMonitor-2.0", RRF_RT_REG_SZ, nullptr, value,
                     &bytes) != ERROR_SUCCESS)
        return {};
    return value;
}
...
bool setStartup(const std::filesystem::path &exe, bool enabled, std::wstring &error)
{
    ...
    if (st == ERROR_SUCCESS)
    {
        st = enabled ? RegSetValueExW(key, L"NativePerfMonitor-2.0", 0, REG_SZ,
                                      reinterpret_cast<const BYTE *>(expected.c_str()),
                                      DWORD((expected.size() + 1) * sizeof(wchar_t)))
                     : RegDeleteValueW(key, L"NativePerfMonitor-2.0");
        RegCloseKey(key);
    }
