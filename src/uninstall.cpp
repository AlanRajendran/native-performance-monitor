...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, LPWSTR, int)
{
    ...
    LocalFree(argv);
    if ((yes || !data.empty() || !result.empty()) && (!isolated || data.empty() || result.empty()))
        return 2;
    if (!yes)
    {
        auto message = L"Remove Native Performance Monitor 1.2 from:\n\n" + folder.wstring() +
                       L"\n\nThis removes this copy's program files, settings and matching optional startup "
                       L"entry. Version 1.1 and unrelated files are kept.";
        if (MessageBoxW(nullptr, message.c_str(), L"Uninstall Native Performance Monitor 1.2",
